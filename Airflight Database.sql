...
-- ADD COUNT(*)
SELECT airline, AVG(capacity) AS avg_capacity ,COUNT(*) AS Total_Count
FROM airflight
GROUP BY airline;
...
-- TOP 5 Airline for Booked_Seats
SELECT airline, SUM(booked_seats) AS total_booked
FROM airflight
GROUP BY airline
ORDER BY SUM(booked_Seats) DESC
LIMIT 5;


-- Name the Airline and its details having booked seats = 0
SELECT * FROM Airflight;

SELECT Airline,Flight_Number,Status,booked_seats
from airflight
...
select airline,status,booked_seats from airflight order by booked_seats desc;


select distinct airline,status,booked_seats from airflight order by booked_seats limit 5;
